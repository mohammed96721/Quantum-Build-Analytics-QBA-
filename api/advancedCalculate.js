	...
	<script>
		...
		document.getElementById('buildingForm').addEventListener('submit', async (e) => {
			...
			const getValue = (id, isFloat = false, isInt = false) => {
				...
				if (!element || !element.value) return isInt ? 0 : isFloat ? 0 : '';
			...
			try {
				...
				const inputs = {
					...
					waffleSlabInfo: !document.getElementById('waffleSlabInfo').classList.contains('hidden'),
				...
				if (!result.success) {
					throw new Error(result.error || 'فشل معالجة البيانات');
				}

				const resultContainer = document.getElementById('resultContainer');
				const resultContent = document.getElementById('resultContent');

				// عرض رسالة "تم المعالجة" فقط
				resultContent.innerHTML = `<p style="font-size: 1rem; font-weight: 600; text-align: center;">تم المعالجة</p>`;
				resultContainer.style.display = 'block';

				// إعداد زر تحميل PDF مع جميع البيانات المعالجة
				document.getElementById('downloadPdf').onclick = () => {
					...
					const addSection = (title, data) => {
						...
						if (!data || !Object.keys(data).length) {
							doc.text('لا توجد بيانات متاحة', 190, y, { align: 'right' });
							y += 10;
							return;
						}

						if (data.sections || data.fields || data.items) {
							const sections = data.sections || (data.fields ? [{ fields: data.fields }] : [{ items: data.items }]);
							sections.forEach(section => {
								if (section.subtitle || section.title) {
									checkPageBreak();
									doc.setFontSize(12);
									doc.text(section.subtitle || section.title, 190, y, { align: 'right' });
									y += 8;
								}
								doc.autoTable({
									startY: y,
									head: [['البيان', 'القيمة']],
									body: (section.items || section.fields || []).map(item => [
										item.name || item.label || '',
										`${item.value || 'غير متوفر'} ${item.unit || ''}`
									]),
									styles: { font: 'Amiri', halign: 'right', fontSize: 10 },
									headStyles: { fillColor: [92, 64, 51], textColor: [255, 255, 255] },
							...
							const body = Object.entries(data).map(([key, value]) => {
								let displayValue = typeof value === 'object' && value ? JSON.stringify(value, null, 2) : value || 'غير متوفر';
					...
					doc.setFontSize(16);
					doc.text('تقرير تكاليف البناء', 105, 10, { align: 'center' });

					const pdfData = result.result.pdfData || {};
					addSection('نبذة عن الشركة', pdfData.companyInfo);
					addSection('البيانات المدخلة', pdfData.inputData);
					addSection('تفاصيل الكميات الهندسية', pdfData.quantities);
					addSection('الحسابات الإنشائية', pdfData.engineering);
					addSection('تفاصيل التكاليف', pdfData.costs);
					addSection('أسعار المواد المعتمدة (2025)', pdfData.materialPrices);

					doc.save('تقرير_البناء.pdf');
				};
			} catch (error) {
				// عرض الأخطاء إذا حدثت
				document.getElementById('resultContent').innerHTML = `<p style="color: #C75656; text-align: center;">خطأ: ${error.message}</p>`;
